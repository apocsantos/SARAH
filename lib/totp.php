...
function base32_decode_custom(string $b32): string {
  ...
  $bits=''; for($i=0;$i<strlen($b32);$i++){ $val=strpos($alphabet,$b32[$i]); if($val===false) continue; $bits.=str_pad(decbin($val),5,'0',STR_PAD_LEFT); }
...
function verify_totp(string $secretB32, string $code, int $window=1, int $period=30): bool {
  ...
  for($i=-$window;$i<=$window;$i++) if(hash_equals(hotp($secret,$slice+$i,6),$code)) return true;
  return false;
}
